...
export function GapAnalysis({ result, questions }: GapAnalysisProps) {
  ...
  const dimensionNameMap = new Map(result.dimensions.map((d) => [d.dimensionId, d.name]));
    ...
    <div className="flex flex-col gap-6">
      ...
      <Card>
        ...
        <p className="mb-4 text-sm text-slate-400">
          Questions scored 0 (not adopted). These represent the highest-impact areas for
          improvement.
        </p>
        {unaddressed.length === 0 ? (
          <p className="text-sm text-green-400">🎉 No completely unaddressed questions found!</p>
        ) : (
          <ul className="space-y-4" role="list" aria-label="Top gaps">
            {unaddressed.map((qs) => {
              ...
              return (
                <li key={qs.questionId} className="border-t border-slate-700/60 pt-3">
      ...
      <Card>
        ...
        <p className="mb-4 text-sm text-slate-400">
          These areas were assessed with low confidence. Consider verifying manually or running with{" "}
          <code className="rounded bg-slate-700 px-1 text-xs text-slate-200">--ai-inference</code>{" "}
          for deeper analysis.
        </p>
            ...
            {lowConfidence.map((qs) => {
              ...
              return (
                <li key={qs.questionId} className="border-t border-slate-700/60 pt-3">
                  ...
                  <p className="mt-1 text-sm text-slate-200">{question.text}</p>
                  {evidenceSummary && (
                    <p className="mt-1 text-xs text-slate-500">📎 {evidenceSummary.summary}</p>
                  )}
                </li>
              );
            })}
