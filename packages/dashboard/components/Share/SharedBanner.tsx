...
export function SharedBanner({ org, date }: SharedBannerProps) {
  ...
  return (
    ...
    >
      ...
      Viewing shared results for <span className="font-semibold text-white">{org}</span> from{" "}
      <span className="font-semibold text-white">{formattedDate}</span>. Confidence and evidence are
      not available in shared view.
    </div>
  );
}
