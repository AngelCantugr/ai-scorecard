 ...
 */
export function computeScorecard(signals, metadata, assessedAt = new Date()) {
    ...
    const dimensionScores = dimensions.map((dimension) => {
        ...
        const questionScores = dimensionQuestions.map((question) => {
    ...
    if (tier === undefined) {
        throw new Error(`No tier found for score ${totalScore} (valid range: ${tiers[0]?.minScore}–${tiers.at(-1)?.maxScore})`);
    }
    ...
    const overallConfidence = measuredQuestionScores.length === 0
        ? 0
